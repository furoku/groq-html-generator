...
function buildSystemPrompt(baseHTML) {
  ...
  cachedSystemPrompt = `ベースHTMLを入力ユーザーに最適化し、デザイン・レイアウト・コピーを自由に変更して完全なHTMLを出力してください。商品名とカート機能は保持。\n\n--- BASE_HTML_START ---\n${baseHTML}\n--- BASE_HTML_END ---`;
...
function updateDocsIndex(filename, userInput) {
  const docsPath = path.join(process.cwd(), 'docs', 'index.html');
  if (!fs.existsSync(docsPath)) return;
  
  try {
    let indexContent = fs.readFileSync(docsPath, 'utf-8');
    
    // Add new entry to examples array and scenarios array
    const exampleMatch = indexContent.match(/const examples = \[([\s\S]*?)\];/);
    const scenarioMatch = indexContent.match(/const scenarios = \[([\s\S]*?)\];/);
    
    if (exampleMatch && scenarioMatch) {
      const currentExamples = exampleMatch[1].trim();
      const currentScenarios = scenarioMatch[1].trim();
      
      const newExample = `            '${filename}'`;
      const newScenario = `            '${userInput.slice(0, 30)}${userInput.length > 30 ? '...' : ''}'`;
      
      // Add to beginning of arrays (most recent first)
      ...
      fs.writeFileSync(docsPath, indexContent, 'utf-8');
    }
  } catch (error) {
    console.warn('Failed to update docs/index.html:', error.message);
  }
}

export function saveHTML(html, { outputDir = path.join(process.cwd(), 'docs', 'output'), prefix = 'generated-page', userInput = '' } = {}) {
  ...
  // Update docs/index.html with new entry
  if (userInput) {
    updateDocsIndex(filename, userInput);
  }
